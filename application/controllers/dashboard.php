...
class Dashboard extends Main {
	...
	public function list_users()
	{
		...
		// $post = $this->User->get_number_of_post();
		...
		$post_numbers  = $this->Post->get_post_numbers_by_id($this->current_user);
		$post_details  = $this->Post->get_post_details_by_id($this->current_user);
	...
	{
		if($this->input->post("form_action")=="Poke")
		{	
			$poke = $this->input->post("one_poke");
			$poke = $poke + 1;
			// var_dump($poke);
			// die();
			$post = $this->Post->update_post($this->input->post(),$poke) ;
			$update_poke = $this->User->update_user($this->input->post("post_created_for"), $poke) ;
